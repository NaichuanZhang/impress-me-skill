#!/usr/bin/env node
/**
 * Deep Research Chat Server
 * Serves the research page and provides an AI chat endpoint
 * powered by Claude Agent SDK for research-context-aware Q&A.
 *
 * Usage:
 *   node server.js
 *
 ...
 *
 * The Agent SDK picks up auth configuration from environment variables
 * automatically — no code changes needed for different providers.
 */
...
async function streamChat(res, systemPrompt, selectedText, question, conversationHistory) {
...
async function startServer() {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && req.url === '/chat') {
            ...
            req.on('end', async () => {
                try {
                    const { selectedText, question, conversationHistory } = JSON.parse(body);
                    ...
                    await streamChat(res, systemPrompt, selectedText, question, conversationHistory);
                } catch (err) {
                    if (!res.headersSent) {
